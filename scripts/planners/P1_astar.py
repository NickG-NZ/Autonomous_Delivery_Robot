...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        if not (self.statespace_lo[0] <= x[0] <= self.statespace_hi[0]
                and self.statespace_lo[1] <= x[1] <= self.statespace_hi[1]):
            free_flag = False
        free_flag = free_flag and self.occupancy.is_free(x)
        return free_flag
        ...
        for horz in [-1, 0, 1]:
            for vert in [-1, 0, 1]:
                if horz == 0 and vert == 0:
                    continue
                x_neigh = (x[0] + horz * self.resolution, x[1] + vert * self.resolution)
