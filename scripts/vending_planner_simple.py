...
class Vending_Planner:
    ...
    def resume_vending_callback(self, msg):
        #rospy.loginfo('Entered resume_vending_callback')
        if 1 or not self.planning:
            self.planning = True
            #rospy.loginfo('resume vending, planning')

            import time
            # TODO: add if statment so that we do not need to sleep all the time and potentially miss commands
            time.sleep(3)
    ...
    def foodmap_callback(self, msg):
        ...
        """
        if 1 or not self.started_vending:
            # updates foodmap
    ...
    def request_callback(self, msg):
        ...
        if not self.planning:
            ...
            self.planning = False
            rospy.loginfo('vending: finished planning')
    ...
    def cmd_callback(self, msg):
        ...
        if 1 or not self.planning:
            ...
            if not self.queue:
                ...
                rospy.loginfo('poped object of queue, current waypoint set to {0}'.format(self.current_waypoint))
